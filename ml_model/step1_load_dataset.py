...
df = df[['name', 'manufacturer_name',
         'price(₹)', 'pack_size_label']].copy()

df.columns = ['name', 'manufacturer',
              'price', 'pack_size']

# ── Step 3: Detect Category from Medicine NAME ──
df = df.dropna(subset=['name'])

def detect_category(name):
    name = str(name).lower()
    if 'syrup' in name:        return 'Syrup'
    elif 'injection' in name:  return 'Injection'
    elif 'drop' in name:       return 'Drops'
    elif 'cream' in name:      return 'Cream'
    elif 'ointment' in name:   return 'Ointment'
    elif 'capsule' in name:    return 'Capsule'
    elif 'gel' in name:        return 'Gel'
    elif 'inhaler' in name:    return 'Inhaler'
    elif 'lotion' in name:     return 'Lotion'
    elif 'suspension' in name: return 'Syrup'
    elif 'powder' in name:     return 'Powder'
    else:                      return 'Tablet'
